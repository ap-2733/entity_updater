...
/**
 * Strips every character that is not an ASCII letter, then lowercases the
 * result.  Used to compare property names in a punctuation-agnostic way so
 * that `_id`, `Id`, and `id` all normalize to `"id"`, and `productId` and
 * `product_id` both normalize to `"productid"`.
 */
...
/**
 * Scans the properties of `type` and returns the name of the best candidate
 * for a unique identifier field, using two-tier priority:
 *
 *   1. Exact match  — normalized name is exactly `"id"` (e.g. `id`, `_id`).
 *   2. Suffix match — normalized name ends with `"id"` (e.g. `productId`,
 *      `userId`).
 *
 * The first exact match wins; if none is found, the first suffix match wins.
 * Returns `undefined` when the type has no id-like property (e.g. wrapper
 * types such as GetProductsByIdApiResponse).
 */
function findIdField(checker: ts.TypeChecker, type: ts.Type): string | undefined {
  let exact: string | undefined;
  let suffix: string | undefined;
  for (const prop of checker.getPropertiesOfType(type)) {
    const norm = normalizeFieldName(prop.getName());
    if (norm === "id") exact ??= prop.getName();
    else if (norm.endsWith("id")) suffix ??= prop.getName();
  }
  return exact ?? suffix;
}

/**
 * Snapshot of a named entity type that carries everything the code generator
 * needs:
 *   - name      the TypeScript type alias name used as the entityType string
 *               and to derive the traverseXxx function name.
 *   - type      the ts.Type as seen at a usage site (has aliasSymbol set).
 *   - idField   the property name to read the entity's id from at runtime.
 */
...
/**
 * Recursively discovers all named entity types reachable from `type` and
 * populates `result` with one EntityInfo per type.
 *
 * Traversal rules:
 *   Union  — recurse into every member (handles `T | undefined`, `A | B`).
 *   Array  — unwrap to the element type and recurse.
 *   Entity — record the type if it has an id field, then recurse into its own
 *            declared properties so that nested entities (e.g. Product →
 *            relatedProducts → Product[]) are collected as well.
 *   Object — walk properties to reach entities embedded in wrapper objects
 *            (e.g. { product?: Product; reviews?: Review[] }).
 *
 * `seenNames` prevents infinite loops when a type refers to itself directly
 * or through a cycle (e.g. Product → relatedProducts → Product[]).  We key
 * by alias name rather than ts.Type identity because the same alias can
 * produce multiple distinct ts.Type instances depending on where it is used.
 */
function collectEntityTypes(
  ...
  if (type.isUnion()) {
    for (const t of type.types) collectEntityTypes(checker, t, result, seenNames);
    return;
  }
  const elem = getArrayElementType(checker, type);
  if (elem) { collectEntityTypes(checker, elem, result, seenNames); return; }
  if (isEntityType(checker, type)) {
    const name = type.aliasSymbol!.getName();
    if (seenNames.has(name)) return; // cycle guard
    seenNames.add(name);
    // getDeclaredTypeOfSymbol gives the canonical object type behind the alias,
    // which is required to reliably iterate properties and find the id field.
    const declared = checker.getDeclaredTypeOfSymbol(type.aliasSymbol!);
    const idField = findIdField(checker, declared);
    // Only register types that have a usable id — types without one (e.g.
    // wrapper types) are still traversed below so their nested entities are found.
    if (idField) result.set(name, { name, type, idField });
    for (const prop of declared.getProperties()) {
      const decl = prop.valueDeclaration ?? prop.declarations?.[0];
      if (decl) collectEntityTypes(checker, checker.getTypeOfSymbolAtLocation(prop, decl), result, seenNames);
    }
    return;
  }
  // Plain (non-entity) object — walk properties to find entities embedded inside
  // wrapper shapes such as { product?: Product; reviews?: Review[] }.
